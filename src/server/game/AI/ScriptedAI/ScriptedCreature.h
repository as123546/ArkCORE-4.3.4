...
struct ScriptedAI: public CreatureAI
{
    ...
    template<class T> inline const T& DUNGEON_MODE(const T& normal5, const T& heroic10)
    {
        switch (m_difficulty)
        {
    ...
    template<class T> inline const T& RAID_MODE(const T& normal10, const T& normal25, const T& heroic10, const T& heroic25)
    {
        switch (m_difficulty)
        {
        case RAID_DIFFICULTY_10MAN_NORMAL:
...
{
    Scripted_NoMovementAI(Creature* creature) :
            ScriptedAI(creature)
    {
    }
