...
void WorldSession::SendBindPoint (Creature *npc)
{
    ...
    // update sql homebind
    CharacterDatabase.PExecute("UPDATE character_homebind SET map = '%u', zone = '%u', position_x = '%f', position_y = '%f', position_z = '%f' WHERE guid = '%u'", _player->GetMapId(), _player->GetAreaId(), _player->GetPositionX(), _player->GetPositionY(), _player->GetPositionZ(), _player->GetGUIDLow());
...
void WorldSession::SendStablePet (uint64 guid)
{
    ...
    m_sendStabledPetCallback.SetFutureResult(CharacterDatabase.AsyncPQuery("SELECT owner, slot, id, entry, level, name FROM character_pet WHERE owner = '%u' AND slot >= '%u' AND slot <= '%u' ORDER BY slot", _player->GetGUIDLow(), PET_SLOT_HUNTER_FIRST, PET_SLOT_STABLE_LAST));
}
...
void WorldSession::HandleStableChangeSlot (WorldPacket & recv_data)
{
    ...
    m_stableChangeSlotCallback.SetFutureResult(CharacterDatabase.PQuery("SELECT slot, entry, id FROM character_pet WHERE owner = '%u' AND id = '%u'", _player->GetGUIDLow(), pet_number));
}
