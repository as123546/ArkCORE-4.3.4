    ...
    for (ConditionList::const_iterator i = conditions.begin(); i != conditions.end(); ++i)
    {
        ...
        if ((*i)->isLoaded())
        {
            ...
            if ((*i)->mReferenceId)          //handle reference
            {
                ...
                {
                    sLog->outDebug(LOG_FILTER_CONDITIONSYS, "IsPlayerMeetToConditionList: Reference template -%u not found", (*i)->mReferenceId);          //checked at loading, should never happen
                }
            }
