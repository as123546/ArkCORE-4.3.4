            ...
            if (m_targets.HasTraj())
            {
                if (Vehicle *vehicle = m_caster->GetVehicleKit())
                    if (Unit *passenger = vehicle->GetPassenger(damage - 1))
                    {
                        ...
                        if (target && target->IsWithinDist2d(&m_targets.m_dstPos, m_spellInfo->GetSpellRadius(effIndex, false) * 2))          // now we use *2 because the location of the seat is not correct
    ...
    if (unitTarget && unitTarget->isAlive() && damage >= 0)
    {
        ...
        else if (m_spellInfo->TargetAuraState == AURA_STATE_SWIFTMEND && unitTarget->HasAuraState(AURA_STATE_SWIFTMEND, m_spellInfo, m_caster))
        {
            ...
            if (Unit* auraCaster = targetAura->GetCaster())
                tickheal = auraCaster->SpellHealingBonus(unitTarget, targetAura->GetSpellInfo(), effIndex, tickheal, DOT);
    ...
    case SPELLFAMILY_HUNTER:
    {
        ...
        case 53209:          // Chimera Shot
        {
            ...
            if (m_caster->HasAuraEffect(53241, 0, 0))
                if (roll_chance_i(sSpellMgr->GetSpellInfo(53241)->Effects[0].BasePoints))
                {
                    m_caster->CastSpell(m_caster->ToPlayer()->GetSelectedUnit(), 88691, true);
                    break;
                }
            if (m_caster->HasAuraEffect(53243, 0, 0))
                if (roll_chance_i(sSpellMgr->GetSpellInfo(53243)->Effects[0].BasePoints))
                {
                    m_caster->CastSpell(m_caster->ToPlayer()->GetSelectedUnit(), 88691, true);
                    break;
                }
...
void Spell::EffectActivateObject (SpellEffIndex effIndex)
{
    ...
    int32 delay_secs = m_spellInfo->Effects[effIndex].MiscValue;
...
void Spell::EffectActivateRune (SpellEffIndex effIndex)
{
    ...
    for (uint32 j = 0; j < MAX_RUNES && count > 0; ++j)
    {
        if (plr->GetRuneCooldown(j) && plr->GetCurrentRune(j) == RuneType(m_spellInfo->Effects[effIndex].MiscValue))
        {
            if (m_spellInfo->Id == 45529)
                if (plr->GetBaseRune(j) != RuneType(m_spellInfo->Effects[effIndex].MiscValue))
                    continue;
    ...
    if (m_spellInfo->Id == 45529 && count > 0)
    {
        ...
        {
            if ((plr->GetRuneCooldown(rune) && plr->GetCurrentRune(rune) == RuneType(m_spellInfo->Effects[effIndex].MiscValue)) && (plr->GetRuneCooldown(rune + 1) && plr->GetCurrentRune(rune + 1) == RuneType(m_spellInfo->Effects[effIndex].MiscValue)))
            {
...
void Spell::EffectWMOChange (SpellEffIndex effIndex)
{
    if (gameObjTarget && gameObjTarget->GetGoType() == GAMEOBJECT_TYPE_DESTRUCTIBLE_BUILDING)
    {
        ...
        int ChangeType = m_spellInfo->Effects[effIndex].MiscValue;
        switch (ChangeType)
        {
        case 0:          // intact
