...
public:
    ...
    virtual bool OnDummyEffect (Unit* /*caster*/, uint32 /*spellId*/, SpellEffIndex /*effIndex*/, Item* /*target*/)
    {
        return false;
    }
...
public:
    ...
    virtual bool OnDummyEffect (Unit* /*caster*/, uint32 /*spellId*/, SpellEffIndex /*effIndex*/, Creature* /*target*/)
    {
        return false;
    }
...
public:
    ...
    virtual bool OnDummyEffect (Unit* /*caster*/, uint32 /*spellId*/, SpellEffIndex /*effIndex*/, GameObject* /*target*/)
    {
        return false;
    }
...
public:
    ...
    virtual bool OnConditionCheck (Condition* /*condition*/, Player* /*player*/, Unit* /*invoker*/)
    {
        return true;
    }
};
    ...
    public:
        ...
        static void AddScript (TScript* const script)
        {
            ...
            for (ScriptMapIterator it = ScriptPointerList.begin(); it != ScriptPointerList.end(); ++it)
            {
                if (it->second == script)
                {
                    sLog->outError("Script '%s' has same memory pointer as '%s'.", script->GetName().c_str(), it->second->GetName().c_str());
            ...
            if (script->IsDatabaseBound())
            {
                ...
                if (id)
                {
                    ...
                    else
                    {
                        ...
                        sLog->outError("Script '%s' already assigned with the same script name, so the script can't work.", script->GetName().c_str());
