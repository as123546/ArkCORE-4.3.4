...
namespace Trinity
{
    ...
    struct MessageDistDeliverer
    {
        ...
        MessageDistDeliverer (WorldObject *src, WorldPacket *msg, float dist, bool own_team_only = false, Player const* skipped = NULL) :
                i_source(src), i_message(msg), i_phaseMask(src->GetPhaseMask()), i_distSq(dist * dist), team((own_team_only && src->GetTypeId() == TYPEID_PLAYER) ? ((Player*) src)->GetTeam() : 0), skipped_receiver(skipped)
        {
        }
        void Visit (PlayerMapType &m);
        void Visit (CreatureMapType &m);
...
// WorldObject check classes
    ...
    public:
        ...
        {
            if (i_funit->GetTypeId() != TYPEID_PLAYER || !((Player*) i_funit)->isHonorOrXPTarget(u) || u->getDeathState() != CORPSE || u->isDeadByDefault() || u->isInFlight() || (u->GetCreatureTypeMask() & (1 << (CREATURE_TYPE_HUMANOID - 1))) == 0 || (u->GetDisplayId() != u->GetNativeDisplayId()))
                return false;
    ...
    public:
        ...
        {
            if (u->getDeathState() != CORPSE || u->isInFlight() || u->HasAuraType(SPELL_AURA_GHOST) || (u->GetDisplayId() != u->GetNativeDisplayId()))
                return false;
        ...
        bool operator() (Creature* u)
        {
            if (u->getDeathState() != CORPSE || u->isInFlight() || u->isDeadByDefault() || (u->GetDisplayId() != u->GetNativeDisplayId()) || (u->GetCreatureTypeMask() & CREATURE_TYPEMASK_MECHANICAL_OR_ELEMENTAL) != 0)
                return false;
    ...
    public:
        ...
        bool operator() (Creature* u)
        {
            if (i_funit->IsFriendlyTo(u) || u->isAlive() || u->isInFlight() || (u->GetCreatureTypeMask() & CREATURE_TYPEMASK_HUMANOID_OR_UNDEAD) == 0)
                return false;
    ...
    public:
        ...
        bool operator() (Creature* u)
        {
            if (i_funit->IsFriendlyTo(u) || u->isAlive() || u->isInFlight() || (u->GetCreatureTypeMask() & CREATURE_TYPEMASK_MECHANICAL_OR_ELEMENTAL) != 0)
                return false;
...
// Unit checks
    ...
    public:
        ...
        bool operator() (Unit* u)
        {
            if (u->isAlive() && u->isInCombat() && !i_obj->IsHostileTo(u) && i_obj->IsWithinDistInMap(u, i_range) && (u->isFeared() || u->isCharmed() || u->isFrozen() || u->HasUnitState(UNIT_STAT_STUNNED) || u->HasUnitState(UNIT_STAT_CONFUSED)))
            {
                return true;
            }
            return false;
    ...
    public:
        ...
        bool operator() (Unit* u)
        {
            return u->isAlive() && i_obj->IsWithinDistInMap(u, i_range) && !i_funit->IsFriendlyTo(u) && i_funit->canSeeOrDetect(u);
        }
    private:
        WorldObject const* i_obj;
        Unit const* i_funit;
