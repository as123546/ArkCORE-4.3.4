...
float HighborneLoc[4][3] = { { 1285.41f, 312.47f, 0.51f }, { 1286.96f, 310.40f,
        1.00f }, { 1289.66f, 309.66f, 1.52f }, { 1292.51f, 310.50f, 1.99f }, };
...
public:
    ...
    bool ChooseReward(Player* /*pPlayer*/, Creature* pCreature,
            const Quest *_Quest, uint32 /*slot*/) {
        if (_Quest->GetQuestId() == 9180) {
            CAST_AI(npc_lady_sylvanas_windrunner::npc_lady_sylvanas_windrunnerAI, pCreature->AI())->LamentEvent =
                    true;
            CAST_AI(npc_lady_sylvanas_windrunner::npc_lady_sylvanas_windrunnerAI, pCreature->AI())->DoPlaySoundToSet(
                    pCreature, SOUND_CREDIT);
            pCreature->CastSpell(pCreature, SPELL_SYLVANAS_CAST, false);

            for (uint8 i = 0; i < 4; ++i)
                pCreature->SummonCreature(ENTRY_HIGHBORNE_LAMENTER,
                        HighborneLoc[i][0], HighborneLoc[i][1], HIGHBORNE_LOC_Y,
                        HighborneLoc[i][2], TEMPSUMMON_TIMED_DESPAWN, 160000);
        }
    ...
    struct npc_lady_sylvanas_windrunnerAI: public ScriptedAI {
        npc_lady_sylvanas_windrunnerAI(Creature *c) :
                ScriptedAI(c) {
        }
        ...
        void UpdateAI(const uint32 diff) {
            if (LamentEvent) {
                if (LamentEvent_Timer <= diff) {
                    DoSummon(ENTRY_HIGHBORNE_BUNNY, me, 10.0f, 3000,
...
public:
    npc_highborne_lamenter() :
            CreatureScript("npc_highborne_lamenter") {
    }
    ...
    struct npc_highborne_lamenterAI: public ScriptedAI {
        npc_highborne_lamenterAI(Creature *c) :
                ScriptedAI(c) {
        }
        ...
        void UpdateAI(const uint32 diff) {
            if (EventMove) {
                if (EventMove_Timer <= diff) {
                    ...
                    me->SendMonsterMoveWithSpeed(me->GetPositionX(),
                            me->GetPositionY(), HIGHBORNE_LOC_Y_NEW, 5000);
                    me->GetMap()->CreatureRelocation(me, me->GetPositionX(),
                            me->GetPositionY(), HIGHBORNE_LOC_Y_NEW,
                            me->GetOrientation());
                    EventMove = false;
                } else
...
public:
    npc_parqual_fintallas() :
            CreatureScript("npc_parqual_fintallas") {
    }
    ...
    bool OnGossipHello(Player* pPlayer, Creature* pCreature) {
        ...
        if (pPlayer->GetQuestStatus(6628) == QUEST_STATUS_INCOMPLETE
                && !pPlayer->HasAura(SPELL_MARK_OF_SHAME)) {
