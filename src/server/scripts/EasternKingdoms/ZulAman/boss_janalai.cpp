...
float FireWallCoords[4][4] = { { -10.13f, 1149.27f, 19, 3.1415f }, { -33.93f,
        1123.90f, 19, 0.5f * 3.1415f }, { -54.80f, 1150.08f, 19, 0 }, { -33.93f,
        1175.68f, 19, 1.5f * 3.1415f } };

float hatcherway[2][5][3] = { { { -87.46f, 1170.09f, 6 },
        { -74.41f, 1154.75f, 6 }, { -52.74f, 1153.32f, 19 }, { -33.37f,
                1172.46f, 19 }, { -33.09f, 1203.87f, 19 } }, { { -86.57f,
        1132.85f, 6 }, { -73.94f, 1146.00f, 6 }, { -52.29f, 1146.51f, 19 }, {
        -33.57f, 1125.72f, 19 }, { -34.29f, 1095.22f, 19 } } };
    ...
    struct boss_janalaiAI: public ScriptedAI {
        ...
        void FireWall() {
            ...
            for (uint8 i = 0; i < 4; ++i) {
                ...
                for (uint8 j = 0; j < WallNum; j++) {
                    if (WallNum == 3)
                        wall = me->SummonCreature(MOB_FIRE_BOMB,
                                FireWallCoords[i][0],
                                FireWallCoords[i][1] + 5 * (j - 1),
                                FireWallCoords[i][2], FireWallCoords[i][3],
                                TEMPSUMMON_TIMED_DESPAWN, 15000);
                    else
                        wall = me->SummonCreature(MOB_FIRE_BOMB,
                                FireWallCoords[i][0] - 2 + 4 * j,
                                FireWallCoords[i][1], FireWallCoords[i][2],
                                FireWallCoords[i][3], TEMPSUMMON_TIMED_DESPAWN,
                                15000);
                    if (wall)
        ...
        void SpawnBombs() {
            ...
            for (int i(0); i < 40; ++i) {
                ...
                Creature* bomb = DoSpawnCreature(MOB_FIRE_BOMB, dx, dy, 0, 0,
                        TEMPSUMMON_TIMED_DESPAWN, 15000);
                if (bomb)
        ...
        bool HatchAllEggs(uint32 uiAction) //1: reset, 2: isHatching all
...

            {
                ...
                Trinity::CreatureListSearcher<
                        Trinity::AllCreaturesOfEntryInRange> searcher(me,
                        templist, check);
        ...
        void Boom() {
...

            {
                ...
                Trinity::AllCreaturesOfEntryInRange check(me, MOB_FIRE_BOMB,
                        100);
                Trinity::CreatureListSearcher<
                        Trinity::AllCreaturesOfEntryInRange> searcher(me,
                        templist, check);
        ...
        void HandleBombSequence() {
            if (BombCount < 40) {
                if (Unit *FireBomb = Unit::GetUnit((*me), FireBombGUIDs[BombCount])) {
                    FireBomb->RemoveFlag(UNIT_FIELD_FLAGS,
                            UNIT_FLAG_NOT_SELECTABLE);
                    DoCast(FireBomb, SPELL_FIRE_BOMB_THROW, true);
                    FireBomb->SetFlag(UNIT_FIELD_FLAGS,
                            UNIT_FLAG_NOT_SELECTABLE);
                }
                ++BombCount;
                if (BombCount == 40) {
        ...
        void UpdateAI(const uint32 diff) {
            ...
            if (BombTimer <= diff) {
                ...
                DoTeleportTo(JanalainPos[0][0], JanalainPos[0][1],
                        JanalainPos[0][2]);
                me->StopMoving();
            ...
            if (!noeggs) {
                if (HealthBelowPct(35)) {
                    ...
                    if (HatchAllEggs(0)) {
                        ...
                        me->SummonCreature(MOB_AMANI_HATCHER,
                                hatcherway[0][0][0], hatcherway[0][0][1],
                                hatcherway[0][0][2], 0,
                                TEMPSUMMON_CORPSE_TIMED_DESPAWN, 10000);
                        me->SummonCreature(MOB_AMANI_HATCHER,
                                hatcherway[1][0][0], hatcherway[1][0][1],
                                hatcherway[1][0][2], 0,
                                TEMPSUMMON_CORPSE_TIMED_DESPAWN, 10000);
                        HatcherTimer = 90000;
                    } else
...
public:
    ...
    struct mob_janalai_firebombAI: public ScriptedAI {
        mob_janalai_firebombAI(Creature *c) :
                ScriptedAI(c) {
        }
...
public:
    ...
    struct mob_janalai_hatcherAI: public ScriptedAI {
        mob_janalai_hatcherAI(Creature *c) :
                ScriptedAI(c) {
            pInstance = c->GetInstanceScript();
        }
        ...
        void UpdateAI(const uint32 diff) {
            if (!pInstance
                    || !(pInstance->GetData(DATA_JANALAIEVENT) == IN_PROGRESS)) {
                me->DisappearAndDie();
                return;
            }
...
public:
    ...
    struct mob_janalai_hatchlingAI: public ScriptedAI {
        mob_janalai_hatchlingAI(Creature *c) :
                ScriptedAI(c) {
            pInstance = c->GetInstanceScript();
        }
        ...
        void Reset() {
            ...
            if (me->GetPositionY() > 1150)
                me->GetMotionMaster()->MovePoint(0,
                        hatcherway[0][3][0] + rand() % 4 - 2,
                        1150.0f + rand() % 4 - 2, hatcherway[0][3][2]);
            else
                me->GetMotionMaster()->MovePoint(0,
                        hatcherway[1][3][0] + rand() % 4 - 2,
                        1150.0f + rand() % 4 - 2, hatcherway[1][3][2]);
    ...
    struct mob_janalai_eggAI: public ScriptedAI {
        ...
        void SpellHit(Unit * /*caster*/, const SpellEntry *spell) {
            if (spell->Id == SPELL_HATCH_EGG) {
                DoSpawnCreature(MOB_HATCHLING, 0, 0, 0, 0,
