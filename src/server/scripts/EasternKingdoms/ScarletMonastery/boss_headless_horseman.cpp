...
static Locations FlightPoint[] = { { 1754.00f, 1346.00f, 17.50f }, { 1765.00f,
        1347.00f, 19.00f }, { 1784.00f, 1346.80f, 25.40f }, { 1803.30f,
        1347.60f, 33.00f }, { 1824.00f, 1350.00f, 42.60f }, { 1838.80f,
        1353.20f, 49.80f }, { 1852.00f, 1357.60f, 55.70f }, { 1861.30f,
        1364.00f, 59.40f }, { 1866.30f, 1374.80f, 61.70f }, { 1864.00f,
        1387.30f, 63.20f }, { 1854.80f, 1399.40f, 64.10f }, { 1844.00f,
        1406.90f, 64.10f }, { 1824.30f, 1411.40f, 63.30f }, { 1801.00f,
        1412.30f, 60.40f }, { 1782.00f, 1410.10f, 55.50f }, { 1770.50f,
        1405.20f, 50.30f }, { 1765.20f, 1400.70f, 46.60f }, { 1761.40f,
        1393.40f, 41.70f }, { 1759.10f, 1386.70f, 36.60f }, { 1757.80f,
        1378.20f, 29.00f }, { 1758.00f, 1367.00f, 19.51f } };

static Locations Spawn[] = { { 1776.27f, 1348.74f, 19.20f }, //spawn point for pumpkin shrine mob
        { 1765.28f, 1347.46f, 17.55f } //spawn point for smoke
};

static const char* Text[] = { "Horseman rise...", "Your time is nigh...",
        "You felt death once...", "Now, know demise!" };
...
public:
    ...
    struct mob_wisp_invisAI: public ScriptedAI {
        ...
        void MoveInLineOfSight(Unit *who) {
            ...
            if (me->IsWithinDist(who, 0.1f, false)
                    && !who->HasAura(SPELL_SQUASH_SOUL))
...
public:
    ...
    struct mob_headAI: public ScriptedAI {
        ...
        void SaySound(int32 textEntry, Unit *pTarget = 0) {
            ...
            Creature *speaker = DoSpawnCreature(HELPER, 0, 0, 0, 0,
...
public:
    boss_headless_horseman() :
            CreatureScript("boss_headless_horseman") {
    }
    ...
    struct boss_headless_horsemanAI: public ScriptedAI {
        boss_headless_horsemanAI(Creature *c) :
                ScriptedAI(c) {
        ...
        void FlyMode() {
            ...
            me->AddUnitMovementFlag(
                    MOVEMENTFLAG_ONTRANSPORT | MOVEMENTFLAG_LEVITATING);
        ...
        void MovementInform(uint32 type, uint32 i) {
            ...
            case 1: {
                if (Creature *smoke = me->SummonCreature(HELPER, Spawn[1].x, Spawn[1].y, Spawn[1].z, 0, TEMPSUMMON_TIMED_DESPAWN, 20000))
                    CAST_AI(mob_wisp_invis::mob_wisp_invisAI, smoke->AI())->SetType(
                            3);
        ...
        void KilledUnit(Unit *plr) {
            if (plr->GetTypeId() == TYPEID_PLAYER) {
                ...
                //maybe possible when player dies from conflagration
                    ...
                    CAST_AI(mob_head::mob_headAI, Head->AI())->SaySound(
                            SAY_PLAYER_DEATH);
            }
        }
        ...
        Player* SelectRandomPlayer(float range = 0.0f, bool checkLoS = true) {
            ...
            for (Map::PlayerList::const_iterator i = PlayerList.begin();
                    i != PlayerList.end(); ++i)
                if ((me->IsWithinLOSInMap(i->getSource()) || !checkLoS)
                        && me->getVictim() != i->getSource()
                        && me->IsWithinDistInMap(i->getSource(), range)
                        && i->getSource()->isAlive())
        ...
        void SpellHitTarget(Unit* unit, const SpellEntry* spell) {
            if (spell->Id == SPELL_CONFLAGRATION
                    && unit->HasAura(SPELL_CONFLAGRATION))
        ...
        void JustDied(Unit* /*killer*/) {
            ...
            if (Creature *wisp = DoSpawnCreature(WISP_INVIS, 0, 0, 0, 0, TEMPSUMMON_TIMED_DESPAWN, 60000))
                CAST_AI(mob_wisp_invis::mob_wisp_invisAI, wisp->AI())->SetType(
                        4);
            if (pInstance)
        ...
        void SpellHit(Unit *caster, const SpellEntry* spell) {
            ...
            if (spell->Id == SPELL_FLYING_HEAD) {
                ...
                caster->GetMotionMaster()->MoveFollow(me, 6,
                        float(urand(0, 5)));
                //DoResetThreat();//not sure if need
                std::list<HostileReference*>::const_iterator itr;
                for (itr = caster->getThreatManager().getThreatList().begin();
                        itr != caster->getThreatManager().getThreatList().end();
                        ++itr) {
                    Unit* pUnit = Unit::GetUnit((*me), (*itr)->getUnitGuid());
                    if (pUnit && pUnit->isAlive() && pUnit != caster)
                        me->AddThreat(pUnit,
                                caster->getThreatManager().getThreat(pUnit));
                }
            }
        }
        ...
        void DamageTaken(Unit *done_by, uint32 &damage) {
            if (damage >= me->GetHealth() && withhead) {
                ...
                if (Head && Head->isAlive()) {
                    ...
                    done_by->ProcDamageAndSpell(me, PROC_FLAG_KILL,
            ...
            if (withhead) {
                ...
                case 0: {
                    if (!IsFlying) {
                        ...
                        if (wp_reached) {
                            wp_reached = false;
                            me->GetMotionMaster()->Clear(false);
                            me->GetMotionMaster()->MovePoint(id,
                                    FlightPoint[id].x, FlightPoint[id].y,
                                    FlightPoint[id].z);
                        }
                    }
                }
                ...
                case 1:
                    ...
                    if (burn <= diff) {
                        if (Creature *flame = me->SummonCreature(HELPER, Spawn[0].x, Spawn[0].y, Spawn[0].z, 0, TEMPSUMMON_TIMED_DESPAWN, 17000))
                            CAST_AI(mob_wisp_invis::mob_wisp_invisAI, flame->AI())->SetType(
                                    2);
                        burned = true;
                    } else
                        burn -= diff;
                ...
                if (regen <= diff) {
                    ...
                    if (me->IsFullHealth() && !returned) {
                        ...
                        if (Head && Head->isAlive()) {
                            CAST_AI(mob_head::mob_headAI, Head->AI())->Phase =
                                    Phase;
...
public:
    mob_pulsing_pumpkin() :
            CreatureScript("mob_pulsing_pumpkin") {
    }
    ...
    struct mob_pulsing_pumpkinAI: public ScriptedAI {
        mob_pulsing_pumpkinAI(Creature *c) :
                ScriptedAI(c) {
        }
        ...
        void Reset() {
            ...
            Despawn();
            Creature *debuff = DoSpawnCreature(HELPER, 0, 0, 0, 0,
                    TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 14500);
            if (debuff) {
                debuff->SetDisplayId(me->GetDisplayId());
                debuff->CastSpell(debuff, SPELL_PUMPKIN_AURA_GREEN, false);
                CAST_AI(mob_wisp_invis::mob_wisp_invisAI, debuff->AI())->SetType(
                        1);
        ...
        void MoveInLineOfSight(Unit *who) {
            if (!who || !who->isTargetableForAttack() || !me->IsHostileTo(who)
                    || me->getVictim())
                return;
...
public:
    go_loosely_turned_soil() :
            GameObjectScript("go_loosely_turned_soil") {
    }
    ...
    bool OnGossipHello(Player* pPlayer, GameObject* soil) {
        ...
        if (Creature *horseman = soil->SummonCreature(HH_MOUNTED, FlightPoint[20].x, FlightPoint[20].y, FlightPoint[20].z, 0, TEMPSUMMON_MANUAL_DESPAWN, 0)) {
            CAST_AI(boss_headless_horseman::boss_headless_horsemanAI, horseman->AI())->PlayerGUID =
                    pPlayer->GetGUID();
...
void mob_head::mob_headAI::Disappear() {
    ...
    if (bodyGUID) {
        ...
        if (body && body->isAlive()) {
            ...
            me->GetMotionMaster()->MoveIdle();
            CAST_AI(boss_headless_horseman::boss_headless_horsemanAI, body->AI())->returned =
                    true;
        }
    }
}
