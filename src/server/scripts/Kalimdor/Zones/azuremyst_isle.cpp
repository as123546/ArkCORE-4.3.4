...
public:
    npc_draenei_survivor() :
            CreatureScript("npc_draenei_survivor") {
    }
    ...
    struct npc_draenei_survivorAI: public ScriptedAI {
        npc_draenei_survivorAI(Creature *c) :
                ScriptedAI(c) {
        }
        ...
        void MoveInLineOfSight(Unit *who) {
            if (CanSayHelp && who->GetTypeId() == TYPEID_PLAYER
                    && me->IsFriendlyTo(who)
                    && me->IsWithinDistInMap(who, 25.0f)) {
                //Random switch between 4 texts
                DoScriptText(RAND(SAY_HELP1, SAY_HELP2, SAY_HELP3, SAY_HELP4),
                        me, who);
            ...
            if (SayThanksTimer) {
                if (SayThanksTimer <= diff) {
                    ...
                    if (Player* pPlayer = Unit::GetPlayer(*me, pCaster)) {
                        DoScriptText(
                                RAND(SAY_HEAL1, SAY_HEAL2, SAY_HEAL3,
                                        SAY_HEAL4), me, pPlayer);

                        pPlayer->TalkedToCreature(me->GetEntry(),
                                me->GetGUID());
                    }

                    me->GetMotionMaster()->Clear();
                    me->GetMotionMaster()->MovePoint(0, -4115.053711f,
                            -13754.831055f, 73.508949f);
...
public:
    ...
    bool OnGossipSelect(Player* pPlayer, Creature* pCreature,
        ...
        if (uiAction == GOSSIP_ACTION_INFO_DEF) {
            ...
            CAST_AI(npc_engineer_spark_overgrind::npc_engineer_spark_overgrindAI, pCreature->AI())->AttackStart(
                    pPlayer);
        }
        return true;
    }
    ...
    struct npc_engineer_spark_overgrindAI: public ScriptedAI {
        npc_engineer_spark_overgrindAI(Creature *c) :
                ScriptedAI(c) {
...
public:
    ...
    struct npc_injured_draeneiAI: public ScriptedAI {
        npc_injured_draeneiAI(Creature *c) :
                ScriptedAI(c) {
        }
...
public:
    ...
    struct npc_geezleAI: public ScriptedAI {
        ...
        void StartEvent() {
            Step = 0;
            EventStarted = true;
            Creature* Spark = me->SummonCreature(MOB_SPARK, SparkPos[0],
                    SparkPos[1], SparkPos[2], 0,
        ...
        uint32 NextStep(uint32 Step) {
            ...
            case 0:
                if (Spark)
                    Spark->GetMotionMaster()->MovePoint(0, -5080.70f,
                            -11253.61f, 0.56f);
            ...
            case 9:
                ...
                if (Spark)
                    Spark->GetMotionMaster()->MovePoint(0, SparkPos[0],
                            SparkPos[1], SparkPos[2]);
                CompleteQuest();
                return 9000;
            case 10:
                if (Spark)
        ...
        void CompleteQuest() {
            ...
            for (std::list<Player*>::const_iterator itr = players.begin();
                    itr != players.end(); ++itr) {
                if ((*itr)->GetQuestStatus(QUEST_TREES_COMPANY)
        ...
        void DespawnNagaFlag(bool despawn) {
            ...
            me->GetGameObjectListWithEntryInGrid(FlagList, GO_NAGA_FLAG,
                    100.0f);

            if (!FlagList.empty()) {
                for (std::list<GameObject*>::const_iterator itr =
                        FlagList.begin(); itr != FlagList.end(); ++itr) {
                    if (despawn) {
...
public:
    npc_nestlewood_owlkin() :
            CreatureScript("npc_nestlewood_owlkin") {
    }

    struct npc_nestlewood_owlkinAI: public ScriptedAI {
        npc_nestlewood_owlkinAI(Creature *c) :
                ScriptedAI(c) {
        }
    ...
    bool EffectDummyCreature(Unit * /*pCaster*/, uint32 spellId,
            uint32 effIndex, Creature *pCreatureTarget) {
        ...
        if (spellId == SPELL_INOCULATE_OWLKIN && effIndex == 0) {
            ...
            CAST_AI(npc_nestlewood_owlkin::npc_nestlewood_owlkinAI, pCreatureTarget->AI())->DespawnTimer =
                    15000;
...
enum eRavegerCage {
    ...
    SPELL_REND = 13443, SPELL_ENRAGING_BITE = 30736,
...
public:
    go_ravager_cage() :
            GameObjectScript("go_ravager_cage") {
    }
...
public:
    npc_death_ravager() :
            CreatureScript("npc_death_ravager") {
    }
    ...
    struct npc_death_ravagerAI: public ScriptedAI {
        npc_death_ravagerAI(Creature *c) :
                ScriptedAI(c) {
        }
...
public:
    npc_stillpine_capitive() :
            CreatureScript("npc_stillpine_capitive") {
    }

    struct npc_stillpine_capitiveAI: public ScriptedAI {
        npc_stillpine_capitiveAI(Creature *c) :
                ScriptedAI(c) {
        }
        ...
        void Reset() {
            FleeTimer = 0;
            GameObject* cage = me->FindNearestGameObject(GO_BRISTELIMB_CAGE,
                    5.0f);
            if (cage)
...
public:
    go_bristlelimb_cage() :
            GameObjectScript("go_bristlelimb_cage") {
    }

    bool OnGossipHello(Player* pPlayer, GameObject* pGo) {
        if (pPlayer->GetQuestStatus(QUEST_THE_PROPHECY_OF_AKIDA)
                == QUEST_STATUS_INCOMPLETE) {
            Creature* pCreature = pGo->FindNearestCreature(
                    NPC_STILLPINE_CAPITIVE, 5.0f, true);
            if (pCreature) {
                DoScriptText(
                        RAND(CAPITIVE_SAY_1, CAPITIVE_SAY_2, CAPITIVE_SAY_3),
                        pCreature, pPlayer);
                pCreature->GetMotionMaster()->MoveFleeing(pPlayer, 3500);
                pPlayer->KilledMonsterCredit(pCreature->GetEntry(),
                        pCreature->GetGUID());
                CAST_AI(npc_stillpine_capitive::npc_stillpine_capitiveAI, pCreature->AI())->FleeTimer =
                        3500;
                return false;
            }
        }
