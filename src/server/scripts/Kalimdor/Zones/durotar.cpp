    ...
    struct npc_lazy_peonAI: public ScriptedAI {
        ...
        void SpellHit(Unit* caster, const SpellEntry * spell) {
            if (spell->Id == SPELL_AWAKEN_PEON
                    ...
                    && CAST_PLR(caster)->GetQuestStatus(QUEST_LAZY_PEONS)
...
public:
    npc_tiger_matriarch_credit() :
            CreatureScript("npc_tiger_matriarch_credit") {
    }

    struct npc_tiger_matriarch_creditAI: public Scripted_NoMovementAI {
        npc_tiger_matriarch_creditAI(Creature* creature) :
        ...
        void UpdateAI(const uint32 diff) {
            ...
            if (events.ExecuteEvent() == EVENT_CHECK_SUMMON_AURA) {
                ...
                GetCreatureListWithEntryInGrid(tigers, me, NPC_TIGER_VEHICLE,
                        15.0f);
                if (!tigers.empty()) {
...
public:
    npc_tiger_matriarch() :
            CreatureScript("npc_tiger_matriarch") {
    }

    struct npc_tiger_matriarchAI: public ScriptedAI {
        npc_tiger_matriarchAI(Creature* creature) :
                ScriptedAI(creature) {
        }
            ...
            if (HealthBelowPct(20)) {
                ...
                if (Unit* vehSummoner = CAST_SUM(attacker)->GetSummoner()) {
                    vehSummoner->AddAura(SPELL_SUMMON_ZENTABRA_TRIGGER,
                            vehSummoner);
                    vehSummoner->CastSpell(vehSummoner, SPELL_SUMMON_ZENTABRA,
                            true);
...
uint32 const trollmodel[] = { 11665, 11734, 11750, 12037, 12038, 12042, 12049,
        12849, 13529, 14759, 15570, 15701, 15702, 1882, 1897, 1976, 2025, 27286,
        2734, 2735, 4084, 4085, 4087, 4089, 4231, 4357, 4358, 4360, 4361, 4362,
        4363, 4370, 4532, 4537, 4540, 4610, 6839, 7037, 9767, 9768 };

class npc_troll_volunteer: public CreatureScript {
public:
    npc_troll_volunteer() :
            CreatureScript("npc_troll_volunteer") {
    }

    struct npc_troll_volunteerAI: public ScriptedAI {
        npc_troll_volunteerAI(Creature* creature) :
                ScriptedAI(creature) {
            Reset();
...
public:
    spell_mount_check() :
            SpellScriptLoader("spell_mount_check") {
    }
    ...
    class spell_mount_check_AuraScript: public AuraScript {
        ...
        void Register() {
            OnEffectPeriodic +=
                    AuraEffectPeriodicFn(spell_mount_check_AuraScript::HandleEffectPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
        }
    };
...
public:
    spell_voljin_war_drums() :
            SpellScriptLoader("spell_voljin_war_drums") {
    }
    ...
    class spell_voljin_war_drums_SpellScript: public SpellScript {
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/) {
            ...
            if (Unit* target = GetHitUnit()) {
                ...
                if (motivate)
                    caster->CastSpell(target, motivate, true, NULL, NULL,
                            caster->GetGUID());
            }
        }
        ...
        void Register() {
            OnEffect +=
                    SpellEffectFn(spell_voljin_war_drums_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
        }
    };
