...
enum Phases {
    PHASE_NULL = 0, PHASE_BIRTH, PHASE_GROUND, PHASE_FLIGHT,
};
...
public:
    ...
    struct boss_sapphironAI: public BossAI {
        boss_sapphironAI(Creature* c) :
                BossAI(c, BOSS_SAPPHIRON), phase(PHASE_NULL) {
            pMap = me->GetMap();
        ...
        void CheckPlayersFrostResist() {
            if (CanTheHundredClub && pMap && pMap->IsDungeon()) {
                ...
                for (Map::PlayerList::const_iterator itr = players.begin();
                        itr != players.end(); ++itr) {
                    if (itr->getSource()->GetResistance(
                            SPELL_SCHOOL_FROST) > MAX_FROST_RESISTANCE) {
        ...
        void EnterPhaseGround() {
            ...
            events.ScheduleEvent(EVENT_CLEAVE, 5000 + rand() % 10000, 0,
                    PHASE_GROUND);
            events.ScheduleEvent(EVENT_TAIL, 5000 + rand() % 10000, 0,
                    PHASE_GROUND);
            events.ScheduleEvent(EVENT_DRAIN, 24000, 0, PHASE_GROUND);
            events.ScheduleEvent(EVENT_BLIZZARD, 5000 + rand() % 5000, 0,
                    PHASE_GROUND);
            ...
            if (phase == PHASE_GROUND) {
                while (uint32 eventId = events.ExecuteEvent()) {
                    ...
                    case EVENT_CLEAVE:
                        ...
                        events.ScheduleEvent(EVENT_CLEAVE,
                                5000 + rand() % 10000, 0, PHASE_GROUND);
                        return;
                    case EVENT_TAIL:
                        DoCastAOE(SPELL_TAIL_SWEEP);
                        events.ScheduleEvent(EVENT_TAIL, 5000 + rand() % 10000,
                                0, PHASE_GROUND);
                        return;
                    case EVENT_DRAIN:
                        DoCastAOE(SPELL_LIFE_DRAIN);
                        events.ScheduleEvent(EVENT_DRAIN, 24000, 0,
                                PHASE_GROUND);
                        return;
                    case EVENT_BLIZZARD: {
                        ...
                        events.ScheduleEvent(EVENT_BLIZZARD,
                                RAID_MODE(20000, 7000), 0, PHASE_GROUND);
                        break;
                    }
                    case EVENT_FLIGHT:
                        phase = PHASE_FLIGHT;
                ...
                if (uint32 eventId = events.ExecuteEvent()) {
                    ...
                    case EVENT_ICEBOLT: {
                        ...
                        for (;
                                i
                                        != me->getThreatManager().getThreatList().end();
                                ++i)
                        ...
                        if (targets.empty())
                            ...
                            std::vector<Unit*>::const_iterator itr =
                                    targets.begin();
                            advance(itr, rand() % targets.size());
                            iceblocks.insert(
                                    std::make_pair((*itr)->GetGUID(), 0));
