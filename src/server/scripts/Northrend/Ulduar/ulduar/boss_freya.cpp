...
public:
    spell_attuned_to_nature_dose_reduction() :
            SpellScriptLoader("spell_attuned_to_nature_dose_reduction") {
    }
    ...
    class spell_attuned_to_nature_dose_reduction_SpellScript: public SpellScript {
        ...
        void HandleScript(SpellEffIndex /*effIndex*/) {
            ...
            case SPELL_ATTUNED_TO_NATURE_2_DOSE_REDUCTION:
                if (target->HasAura(GetEffectValue()))
                    for (uint8 n = 0; n < 2; n++)
                        target->RemoveAuraFromStack(GetEffectValue(), 0,
        ...
        void Register() {
            OnEffect +=
                    SpellEffectFn(spell_attuned_to_nature_dose_reduction_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };
