...
enum eCreatures {
    NPC_IRON_CONSTRUCT = 33121, NPC_GROUND_SCORCH = 33221
};
...
const Position Pos[20] = { { 630.366f, 216.772f, 360.891f, 3.001970f }, {
        630.594f, 231.846f, 360.891f, 3.124140f }, { 630.435f, 337.246f,
        360.886f, 3.211410f }, { 630.493f, 313.349f, 360.886f, 3.054330f }, {
        630.444f, 321.406f, 360.886f, 3.124140f }, { 630.366f, 247.307f,
        360.888f, 3.211410f }, { 630.698f, 305.311f, 360.886f, 3.001970f }, {
        630.500f, 224.559f, 360.891f, 3.054330f }, { 630.668f, 239.840f,
        360.890f, 3.159050f }, { 630.384f, 329.585f, 360.886f, 3.159050f }, {
        543.220f, 313.451f, 360.886f, 0.104720f }, { 543.356f, 329.408f,
        360.886f, 6.248280f }, { 543.076f, 247.458f, 360.888f, 6.213370f }, {
        543.117f, 232.082f, 360.891f, 0.069813f }, { 543.161f, 305.956f,
        360.886f, 0.157080f }, { 543.277f, 321.482f, 360.886f, 0.052360f }, {
        543.316f, 337.468f, 360.886f, 6.195920f }, { 543.280f, 239.674f,
        360.890f, 6.265730f }, { 543.265f, 217.147f, 360.891f, 0.174533f }, {
        543.256f, 224.831f, 360.891f, 0.122173f } };

class boss_ignis: public CreatureScript {
public:
    boss_ignis() :
            CreatureScript("boss_ignis") {
    }
    ...
    struct boss_ignis_AI: public BossAI {
        boss_ignis_AI(Creature *pCreature) :
                BossAI(pCreature, TYPE_IGNIS), vehicle(me->GetVehicleKit()) {
            assert(vehicle);
            ...
            if (instance) {
                ...
                if (Shattered)
                    instance->DoCompleteAchievement(
                            RAID_MODE(ACHIEVEMENT_SHATTERED_10,
                                    ACHIEVEMENT_SHATTERED_25));
                // Stokin' the Furnace
                if (EncounterTime <= MAX_ENCOUNTER_TIME)
                    instance->DoCompleteAchievement(
                            RAID_MODE(ACHIEVEMENT_STOKIN_THE_FURNACE_10,
                                    ACHIEVEMENT_STOKIN_THE_FURNACE_25));
            }
        }
        ...
        void UpdateAI(const uint32 diff) {
            ...
            while (uint32 eventId = events.ExecuteEvent()) {
                ...
                case EVENT_SLAG_POT:
                    ...
                    events.RescheduleEvent(EVENT_SLAG_POT,
                            RAID_MODE(30000, 15000));
                    break;
                case EVENT_GRAB_POT:
                ...
                case EVENT_SCORCH:
                    ...
                    if (Unit *pTarget = me->getVictim())
                        me->SummonCreature(NPC_GROUND_SCORCH,
                ...
                case EVENT_CONSTRUCT:
                    ...
                    DoSummon(NPC_IRON_CONSTRUCT, Pos[rand() % 20], 30000,
                            TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT);
                    DoCast(SPELL_STRENGHT);
                    DoCast(me, SPELL_ACTIVATE_CONSTRUCT);
                    events.RescheduleEvent(EVENT_CONSTRUCT,
...
public:
    ...
    struct npc_iron_constructAI: public ScriptedAI {
        npc_iron_constructAI(Creature* pCreature) :
...
public:
    npc_scorch_ground() :
            CreatureScript("npc_scorch_ground") {
    }
    ...
    struct npc_scorch_groundAI: public ScriptedAI {
        npc_scorch_groundAI(Creature* pCreature) :
                ScriptedAI(pCreature) {
            me->SetFlag(
                    UNIT_FIELD_FLAGS,
                    UNIT_FLAG_DISABLE_MOVE | UNIT_FLAG_NOT_SELECTABLE
                            | UNIT_FLAG_PACIFIED);
        ...
        void MoveInLineOfSight(Unit* pWho) {
            if (!Heat) {
                if (pWho->GetEntry() == NPC_IRON_CONSTRUCT) {
                    if (!pWho->HasAura(SPELL_HEAT)
                            || !pWho->HasAura(SPELL_MOLTEN)) {
...
public:
    spell_ignis_slag_pot() :
            SpellScriptLoader("spell_ignis_slag_pot") {
    }
    ...
    class spell_ignis_slag_pot_AuraScript: public AuraScript {
        ...
        bool Validate(SpellInfo const * /*spellEntry*/) {
            if (!sSpellMgr->GetSpellInfo(SPELL_SLAG_POT_DAMAGE))
                return false;
            if (!sSpellMgr->GetSpellInfo(SPELL_SLAG_POT))
                return false;
        ...
        void Register() {
            OnEffectPeriodic +=
                    AuraEffectPeriodicFn(spell_ignis_slag_pot_AuraScript::HandleEffectPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
        }
    };
