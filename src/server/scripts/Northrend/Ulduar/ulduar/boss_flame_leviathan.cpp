...
enum Seats {
    SEAT_PLAYER = 0, SEAT_TURRET = 1, SEAT_DEVICE = 2, SEAT_CANNON = 7,
};

enum Vehicles {
    VEHICLE_SIEGE = 33060, VEHICLE_CHOPPER = 33062, VEHICLE_DEMOLISHER = 33109,
};
...
static Position Center[] = { { 354.8771f, -12.90240f, 409.803650f, 0.0f }, };

const Position PosSiege[5] = { { -814.59f, -64.54f, 429.92f, 5.969f }, {
        -784.37f, -33.31f, 429.92f, 5.096f }, { -808.99f, -52.10f, 429.92f,
        5.668f }, { -798.59f, -44.00f, 429.92f, 5.663f }, { -812.83f, -77.71f,
        429.92f, 0.046f }, };

const Position PosChopper[5] = { { -717.83f, -106.56f, 430.02f, 0.122f }, {
        -717.83f, -114.23f, 430.44f, 0.122f }, { -717.83f, -109.70f, 430.22f,
        0.122f }, { -718.45f, -118.24f, 430.26f, 0.052f }, { -718.45f, -123.58f,
        430.41f, 0.085f }, };

const Position PosDemolisher[5] = { { -724.12f, -176.64f, 430.03f, 2.543f }, {
        -766.70f, -225.03f, 430.50f, 1.710f }, { -729.54f, -186.26f, 430.12f,
        1.902f }, { -756.01f, -219.23f, 430.50f, 2.369f }, { -798.01f, -227.24f,
        429.84f, 1.446f }, };
...
public:
    ...
    struct boss_flame_leviathanAI: public BossAI {
        boss_flame_leviathanAI(Creature* pCreature) :
                BossAI(pCreature, TYPE_LEVIATHAN), vehicle(
                        pCreature->GetVehicleKit()) {
            assert(vehicle);
            uiActiveTowers = 4;
            uiShutdown = 0;
            ActiveTowers = false;
            ...
            if (!bReset) {
                if (ActiveTowers) {
                    if (towerOfStorms) {
                        ...
                        events.ScheduleEvent(EVENT_THORIM_S_HAMMER,
                                35 * IN_MILLISECONDS);
                    }

                    if (towerOfFlames) {
                        me->AddAura(SPELL_BUFF_TOWER_OF_FLAMES, me);
                        events.ScheduleEvent(EVENT_MIMIRON_S_INFERNO,
                                70 * IN_MILLISECONDS);
                    }

                    if (towerOfFrost) {
                        me->AddAura(SPELL_BUFF_TOWER_OF_FR0ST, me);
                        events.ScheduleEvent(EVENT_HODIR_S_FURY,
                                105 * IN_MILLISECONDS);
                    }

                    if (towerOfLife) {
                        me->AddAura(SPELL_BUFF_TOWER_OF_LIFE, me);
                        events.ScheduleEvent(EVENT_FREYA_S_WARD,
                                140 * IN_MILLISECONDS);
                    }

                    if (!towerOfLife && !towerOfFrost && !towerOfFlames
                            && !towerOfStorms)
        ...
        void InstallAdds(bool bReset = false) {
            if (!bReset) {
                ...
                if (lSeats.empty())
                    return;
                for (std::list<Creature*>::const_iterator itr = lSeats.begin();
                        itr != lSeats.end(); itr++) {
            ...
            if (ActiveTowers) {
                ...
                case 4:
                    instance->DoCompleteAchievement(
                            RAID_MODE(ACHIEV_10_ORBIT_UARY,
                                    ACHIEV_25_ORBIT_UARY));
                case 3:
                    instance->DoCompleteAchievement(
                            RAID_MODE(ACHIEV_10_NUKED_FROM_ORBIT,
                                    ACHIEV_25_NUKED_FROM_ORBIT));
                case 2:
                    instance->DoCompleteAchievement(
                            RAID_MODE(ACHIEV_10_ORBITAL_DEVASTATION,
                                    ACHIEV_25_ORBITAL_DEVASTATION));
                case 1:
                    instance->DoCompleteAchievement(
                            RAID_MODE(ACHIEV_10_ORBITAL_BOMBARDMENT,
                                    ACHIEV_25_ORBITAL_BOMBARDMENT));
                }
            }
        }
        ...
        void UpdateAI(const uint32 diff) {
            ...
            case EVENT_PURSUE:
                DoScriptText(RAND(SAY_TARGET_1, SAY_TARGET_2, SAY_TARGET_3),
                        me);
                if (Unit* pTarget = SelectTarget(SELECT_TARGET_RANDOM)) {
                    if (pTarget->GetTypeId() == TYPEID_PLAYER
                            || pTarget->HasFlag(UNIT_NPC_FLAGS,
                                    UNIT_NPC_FLAG_PLAYER_VEHICLE)) {
                        me->AddAura(SPELL_PURSUED, pTarget);
                        me->MonsterTextEmote(EMOTE_PURSUE, pTarget->GetGUID(),
                                true);
                    }
                }
            ...
            case EVENT_SHUTDOWN:
                DoScriptText(
                        RAND(SAY_OVERLOAD_1, SAY_OVERLOAD_2, SAY_OVERLOAD_3),
                        me);
                InstallAdds(true);
            ...
            case EVENT_MIMIRON_S_INFERNO: // Tower of Flames
                me->SummonCreature(MOB_MIMIRON_BEACON, 390.93f, -13.91f,
                        409.81f);
        ...
        void DoAction(const int32 uiAction) {
            ...
            if (uiAction == 10) {
                ...
                me->GetMotionMaster()->MoveCharge(354.8771f, -12.90240f,
                        409.803f); //position center
                me->SetReactState(REACT_AGGRESSIVE);
                me->RemoveFlag(
                        UNIT_FIELD_FLAGS,
                        UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE
...
public:
    boss_flame_leviathan_seat() :
            CreatureScript("boss_flame_leviathan_seat") {
    }
    ...
    struct boss_flame_leviathan_seatAI: public PassiveAI {
        boss_flame_leviathan_seatAI(Creature* pCreature) :
                PassiveAI(pCreature), vehicle(pCreature->GetVehicleKit()) {
            assert(vehicle);
#ifdef BOSS_DEBUG
            me->SetReactState(REACT_AGGRESSIVE);
#endif
        ...
        void PassengerBoarded(Unit* who, int8 seatId, bool apply) {
            ...
            if (seatId == SEAT_PLAYER) {
                ...
                if (Unit* pDevice = CAST_CRE(vehicle->GetPassenger(SEAT_DEVICE))) {
                    ...
                    pDevice->RemoveFlag(UNIT_FIELD_FLAGS,
...
public:
    boss_flame_leviathan_defense_cannon() :
            CreatureScript("boss_flame_leviathan_defense_cannon") {
    }
    ...
    struct boss_flame_leviathan_defense_cannonAI: public ScriptedAI {
        boss_flame_leviathan_defense_cannonAI(Creature* pCreature) :
        ...
        bool CanAIAttack(const Unit *who) const {
            if (who->GetTypeId() != TYPEID_PLAYER || !who->GetVehicle()
                    || who->GetVehicleBase()->GetEntry() != 33114)
                return false;
            return true;
        }
    };
...
public:
    ...
    struct boss_flame_leviathan_defense_turretAI: public TurretAI {
        boss_flame_leviathan_defense_turretAI(Creature *c) :
                TurretAI(c) {
        }
...
public:
    ...
    struct boss_flame_leviathan_overload_deviceAI: public PassiveAI {
        boss_flame_leviathan_overload_deviceAI(Creature* pCreature) :
                PassiveAI(pCreature) {
            ...
            if (param == EVENT_SPELLCLICK) {
                if (me->GetVehicle()) {
                    ...
                    if (Unit* pPlayer = me->GetVehicle()->GetPassenger(SEAT_PLAYER)) {
                        me->GetVehicleBase()->CastSpell(pPlayer,
                                SPELL_SMOKE_TRAIL, true);
                        pPlayer->GetMotionMaster()->MoveKnockbackFrom(
...
public:
    ...
    struct boss_flame_leviathan_safety_containerAI: public PassiveAI {
        boss_flame_leviathan_safety_containerAI(Creature* pCreature) :
                PassiveAI(pCreature) {
        }
...
public:
    ...
    struct npc_mechanoliftAI: public PassiveAI {
        npc_mechanoliftAI(Creature* pCreature) :
                PassiveAI(pCreature), vehicle(pCreature->GetVehicleKit()) {
            assert(vehicle);
        }
        ...
        void MovementInform(uint32 /*type*/, uint32 id) {
            if (id == 1) {
                Creature* pContainer = me->FindNearestCreature(MOB_CONTAINER, 5,
                        true);
                if (pContainer)
        ...
        void UpdateAI(const uint32 diff) {
            if (MoveTimer <= diff) {
                if (me->GetVehicleKit()->HasEmptySeat(-1)) {
                    Creature* pContainer = me->FindNearestCreature(
                            MOB_CONTAINER, 50, true);
                    if (pContainer && !pContainer->GetVehicle())
                        me->GetMotionMaster()->MovePoint(1,
                                pContainer->GetPositionX(),
                                pContainer->GetPositionY(),
                                pContainer->GetPositionZ());
                }
...
public:
    spell_pool_of_tar() :
            CreatureScript("spell_pool_of_tar") {
    }
    ...
    struct spell_pool_of_tarAI: public PassiveAI {
        spell_pool_of_tarAI(Creature* pCreature) :
                PassiveAI(pCreature) {
...
public:
    ...
    struct npc_colossusAI: public ScriptedAI {
        npc_colossusAI(Creature* pCreature) :
                ScriptedAI(pCreature) {
            instance = pCreature->GetInstanceScript();
        }
        ...
        void JustDied(Unit* /*Who*/) {
            if (me->GetHomePosition().IsInDist(Center, 50.f))
                instance->SetData(TYPE_COLOSSUS,
                        instance->GetData(TYPE_COLOSSUS) + 1);
        }
...
public:
    npc_thorims_hammer() :
            CreatureScript("npc_thorims_hammer") {
    }
    ...
    struct npc_thorims_hammerAI: public ScriptedAI {
        npc_thorims_hammerAI(Creature* pCreature) :
                ScriptedAI(pCreature) {
            me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
...
public:
    npc_mimirons_inferno() :
            CreatureScript("npc_mimirons_inferno") {
    }
    ...
    struct npc_mimirons_infernoAI: public npc_escortAI {
        npc_mimirons_infernoAI(Creature* pCreature) :
            ...
            if (!HasEscortState(STATE_ESCORT_ESCORTING)) {
                ...
                if (infernoTimer <= diff) {
                    if (Creature* pTrigger = DoSummonFlyer(NPC_MIMIRON_TARGET_BEACON, me, 20, 0, 1000, TEMPSUMMON_TIMED_DESPAWN)) {
                        pTrigger->CastSpell(me->GetPositionX(),
                                me->GetPositionY(), me->GetPositionZ(),
                                SPELL_MIMIRON_S_INFERNO, true);
                        infernoTimer = 2000;
                    }
                } else
                    infernoTimer -= diff;
...
public:
    npc_hodirs_fury() :
            CreatureScript("npc_hodirs_fury") {
    }
    ...
    struct npc_hodirs_furyAI: public ScriptedAI {
        npc_hodirs_furyAI(Creature* pCreature) :
                ScriptedAI(pCreature) {
            me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
...
public:
    npc_freyas_ward() :
            CreatureScript("npc_freyas_ward") {
    }
    ...
    struct npc_freyas_wardAI: public ScriptedAI {
        npc_freyas_wardAI(Creature* pCreature) :
                ScriptedAI(pCreature) {
            me->AddAura(AURA_DUMMY_GREEN, me);
        }
...
public:
    npc_freya_ward_summon() :
            CreatureScript("npc_freya_ward_summon") {
    }
    ...
    struct npc_freya_ward_summonAI: public ScriptedAI {
        npc_freya_ward_summonAI(Creature* pCreature) :
                ScriptedAI(pCreature) {
...
public:
    ...
    bool OnGossipSelect(Player* pPlayer, Creature* pCreature,
        ...
        case GOSSIP_ACTION_INFO_DEF + 2:
            ...
            if (Creature* pLeviathan = instance->instance->GetCreature(instance->GetData64(TYPE_LEVIATHAN))) {
                CAST_AI(boss_flame_leviathan::boss_flame_leviathanAI, (pLeviathan->AI()))->DoAction(
                        0); //enable hard mode activating the 4 additional events spawning additional vehicles
                ...
                if (Creature* Delorah = pCreature->FindNearestCreature(NPC_DELORAH, 1000, true)) {
                    if (Creature* Branz = pCreature->FindNearestCreature(NPC_BRANZ_BRONZBEARD, 1000, true)) {
                        Delorah->GetMotionMaster()->MovePoint(0,
                                Branz->GetPositionX() - 4,
                                Branz->GetPositionY(), Branz->GetPositionZ());
    ...
    struct npc_lorekeeperAI: public ScriptedAI {
        npc_lorekeeperAI(Creature* pCreature) :
                ScriptedAI(pCreature) {
            instance = pCreature->GetInstanceScript();
        }
        ...
        void DoAction(const int32 uiAction) {
            ...
            if (uiAction == 0) {
                for (int32 i = 0; i < RAID_MODE(2, 5); ++i)
                    DoSummon(VEHICLE_SIEGE, PosSiege[i], 3000,
                            TEMPSUMMON_CORPSE_TIMED_DESPAWN);
                for (int32 i = 0; i < RAID_MODE(2, 5); ++i)
                    DoSummon(VEHICLE_CHOPPER, PosChopper[i], 3000,
                            TEMPSUMMON_CORPSE_TIMED_DESPAWN);
                for (int32 i = 0; i < RAID_MODE(2, 5); ++i)
                    DoSummon(VEHICLE_DEMOLISHER, PosDemolisher[i], 3000,
                            TEMPSUMMON_CORPSE_TIMED_DESPAWN);
                return;
            }
        }
...
public:
    go_ulduar_tower() :
            GameObjectScript("go_ulduar_tower") {
    }
    ...
    void OnDestroyed(Player* /*pPlayer*/, GameObject* pGO, uint32 /*value*/) {
        ...
        Creature* trigger = pGO->FindNearestCreature(
                NPC_ULDUAR_GAUNTLET_GENERATOR, 15.0f, true);
        if (trigger)
...
public:
    at_RX_214_repair_o_matic_station() :
            AreaTriggerScript("at_RX_214_repair_o_matic_station") {
    }
    ...
    bool OnTrigger(Player* pPlayer, const AreaTriggerEntry* /*pAt*/) {
        if (Creature* vehicle = pPlayer->GetVehicleCreatureBase()) {
            if (!vehicle->HasAura(SPELL_AUTO_REPAIR)) {
                pPlayer->MonsterTextEmote(EMOTE_REPAIR, pPlayer->GetGUID(),
                        true);
